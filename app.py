...
class BotBalancer:
    """
    This class aims to balance the load of the bots given in interview question
    ...
    def parse_values(self):
        ...
        for line in self.instructions:
            if val := re.match(r'value (\d+) goes to bot (\d+)', line):
                self.database.execute(f"""INSERT INTO allocations(chip,bot) VALUES({val.group(2)},{val.group(1)})""")
    ...
    def part1(self, chip_val_1, chip_val_2):
        ...
        for bot, chips in bins.items():
            if type(chips) == tuple:
                chip_1 = chips[0]
                chip_2 = chips[1]
                self.database.execute(f"""INSERT INTO bots(bot_id,chip_1_value,chip_2_value) VALUES({bot.replace("bot",
                                                                                                                 "")},{chip_1},{chip_2})""")
                self.database.commit()
            else:
    ...
    def part2(self, output_id_list):
        ...
        output_id <= {output_id_list[-1]}""").fetchall()
        result = 1
